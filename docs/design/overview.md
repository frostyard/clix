...
A reused flag is reset by `resetOwnedFlag` before the loop moves on: it sets the flag's `Value` back to `DefValue` — which also resets the bound package-level bool, since pflag's `boolValue` is the `*bool` target itself — and clears `Changed` to `false`. Without this, a value or an explicit-flag marker left by an earlier invocation would carry into the next one: cobra's parser only ever sets `Changed` to `true` for a flag the current invocation's arguments name, it never clears a stale `true` from before, so a later invocation that passes no `--json` would still observe `JSONOutput == true` from an earlier `--json` invocation, and `BindViper`'s command-line-over-viper precedence (which trusts `Changed` to mean "this invocation set it explicitly") would wrongly suppress viper's value too. Pinned by `TestRunTwiceOnSameRootJSONTrueThenFalseIsolatesState` and `TestRunTwice_StaleChangedDoesNotSuppressViper` in `clix_test.go`.

**`BindViper(cmd)`** binds all four flags to viper keys (`json`, `verbose`, `dry-run`, `silent`). Optional — call in `PersistentPreRunE` if the consuming CLI uses viper for config. It resolves each flag through the unexported `lookupFlag(cmd, name)`: `cmd.Flags()` (holds merged persistent flags once parsing has run), then `cmd.PersistentFlags()`, then `cmd.InheritedFlags()` — so it works when cobra invokes the root's `PersistentPreRunE` with `cmd` set to an executing subcommand, whose only view of the clix flags is inherited (pinned by `TestBindViper_FromSubcommandPreRun`). A flag that resolves nowhere returns `clix: BindViper: --<name> is not registered on "<cmd>"; call App.Run on the root command first` instead of viper's bare `flag for "json" is nil` (pinned by `TestBindViper_UnregisteredFlagError`). A nil `cmd` is rejected at the exported boundary with `clix: BindViper: command is nil` before any lookup, so a mis-wired `PersistentPreRunE` returns an error instead of panicking (pinned by `TestBindViper_NilCommand`).
...
| **test** | The repository gate `make ci` — run `make ci` to reproduce this job locally. It runs, in order: `make verify` (`go mod tidy` drift check, `gofmt`, `go vet`, `lint-version-check`, `golangci-lint` at the `mise.toml` pin, and `go test ./...`), `make test-race` (`go test -race ./...`), the coverage-instrumented `go test -v -coverprofile=coverage.out -covermode=atomic -coverpkg=github.com/frostyard/clix ./...` — unit tests plus the `tests/e2e/` suite, then `make test-coverage-check` and `make coverage-check` (95.0% statement-coverage floor on the `clix` package, plus a guard that fails and names any production function reported at 0.0% coverage even when the aggregate clears the floor) |
